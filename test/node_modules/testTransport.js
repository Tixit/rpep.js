...
var ConnectionObject = function(connectionInfo) {
    var isOpen  = true
    var messageHandler, errorHandlers = [], closeHandlers = [], openHandlers = []
    ...
    var result = {
        ...
        close: function() {
            if(isOpen && closeHandlers.length > 0) {
                setTimeout(function() { // ensure asynchronicity
                    connectionInfo.emit('close')
                })
                callAllHandlers(closeHandlers)
                isOpen = false
            }
        },
        onOpen: function(cb) {
    ...
    if(connectionInfo.options.drop) {
        result.drop = function() {
            if(isOpen) callAllHandlers(closeHandlers)
        }
    }
